...
- [x] **N1. 고정밀 화자분리 배포 계약 일치** — 2026-08-11 완료 (`40cd86d`)
  ...
  **해결**: 내려받기 UI 대신 **모델 21MB를 앱 번들에 담았다.** CLI가 번들을 먼저 보고
  없을 때만 홈 캐시로 내려간다. 홈 캐시를 감추고 네트워크를 차단한 상태에서 완주 확인.
  덤: 네트워크·샌드박스·심사 위험이 함께 사라져 D1의 가장 큰 리스크가 줄었다.
  부수 정정: manifest 라이선스가 틀렸다 — 모델은 Apache-2.0이 아니라 **CC-BY-4.0**
  (pyannote community-1 파생). 출처 표시 의무라 `NOTICE.txt`를 번들에 함께 넣었다.

- [x] **N2. 고정밀 실패 사유를 사용자에게 보이기** — 2026-08-11 완료 (`40cd86d`)
  엔진은 `binary_missing`·`model_missing`·`inference_failed`를 구조화 이벤트로 남기는데
  화면에 나오지 않는다. **사용자는 왜 기본 분리로 내려갔는지 알 수 없다** — 조용한 품질 저하다.
  **해결**: `JobRunner`가 `@evt` JSON 이벤트를 읽고(문구 정규식이 아니라 JSON이라
  로그 문구가 바뀌어도 안 깨진다), 라이브 대본 위에 경고 줄로 사유를 띄운다.
  모델 없음·검증 실패·시간 초과를 각각 다른 문장으로, 모르는 코드는 원문과 함께.

- [~] **C2. 기본 분리를 고정밀로 대체**
  resemblyzer는 실측 한계다(마진 0.058, 지도 분류 상한 82.8%). 클러스터링 개선은 소진.
  → **N1 완료 후 고정밀을 기본 경로로.** 단 UI만으로는 부족하고 배포 계약 전체가 필요하다.
  N1이 끝나 **모든 맥에서 고정밀이 가능해졌다.** 남은 것은 기본값을 켤지 결정하는 것 —
  고정밀은 115초를 2.9초에 처리하므로 속도는 문제가 아니다.
  선행: 없음(남은 건 판단) · 위험: 낮음
...
- [x] **C3. 겹침 발화 2줄 자막** — 2026-08-11 v1 완료 (엔진 `c2d40e7` · 앱 `e1f38ed`)
  **해결(v1)**: coreml 겹침 → 분석 캐시 → 카드 둘째 줄("- …") 합성. 토글은 화자
  분리 아래, 스토리라인 묶기와 배타, 재분석 없이 켜고 끔. 실측 17분: 겹침 47곳 중
  4장에 2줄 — STT가 한 줄기로만 전사해 **두 텍스트가 모두 있는 겹침만** 가능하다.
  묻힌 음성까지 살리려면 음원 분리(v2). 시안은 사용자 지시로 추후.
